...
/////////////ファイルを作成してゲットした画像のバイトデータをそのファイルに書き込む/////////////////
	 public static void ProcessImage(byte[] decoded){ ///引数にByte列
		try {		
			FileOutputStream bf = new FileOutputStream("/media/docker_shared/mono_develop/img/getImg.jpg");
			bf.write(decoded);
			bf.close();
		}
...
/////////////から作成されたファイルのインポート////////////////////
	 public static byte[] ProcessImport() throws Exception {
		 byte[] b = new byte[1];
	 	 FileInputStream fis = new FileInputStream("/media/docker_shared/mono_develop/img/kansei.jpg");
